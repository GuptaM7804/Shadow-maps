...
window.updateLightRotate = function() {
    ...
    curR = curR + (currLightRotate - prevRLight);
    prevRLight = currLightRotate;
}
...
function updateProjectionMatrix() {
    ...
    if (currProj == "perspective") {
        ...
        var zoom = 4000 - (currZoom / 100.0) * 4000 * 0.99;
        projectionMatrix = orthographicMatrix(-aspect * zoom, aspect * zoom, -1 * zoom, 1 * zoom, -1, 50000);
    }
}
...
function updateViewMatrix(centroid){
    ...
    var perspective = lookAt(eye, centroid, [0, 0, 1]);
    var position = translateMatrix(0, 0, -zoom);
    viewMatrix = multiplyArrayOfMatrices([position, perspective]);
}
...
function updateLightViewMatrix(centroid) {
    ...
    if (!displayShadowmap) {
        var perspective = lookAt(add(centroid, [-x, y, 2000]), centroid, [0, 0, 1]);
    } else {
        var perspective = lookAt(add(centroid, [x, y, 2000]), centroid, [0, 0, 1]);
    }
    lightViewMatrix = perspective;
}

function updateLightProjectionMatrix() {
    // TODO: Light projection matrix
    var maxZ = 2400;
    lightProjectionMatrix = orthographicMatrix(-1 * maxZ, 1 * maxZ, -1 * maxZ, 1 * maxZ, -1, 20000);
}
...
var zoomation = document.getElementById("zoom");
...
function mouseMove() {
    ...
    canvas.addEventListener('mousedown', e => {
        ...
        sliderx = x / canvas.clientWidth * 360;
        slidery = y / canvas.clientHeight * 100;

        rotation.value = sliderx;
        zoomation.value = slidery;
